...
function MagicCards() {
    ...
    return (
        // INFO SECTION
        <div className='flex flex-col text-center text-2xl bg-black w-screen h-screen'>
            <div className='bg-violet-300 mx-auto rounded p-4'>
                <h1>Instructions are simple. Click the random pack button</h1>
                <h1>Some card images won't load, and that is an error with the calls to the API</h1>
                <h1>Receive your random pack and wait for it to load</h1>
                <h1><strong>IT MAY TAKE SOME TIME TO GET THE CARDS - PATIENCE, PLEASE =D</strong></h1>
            ...
            <h1 className='mx-auto bg-white rounded'>Total cards: {choosenPack === null ? <>{0}</> : <>{choosenPack.cards.length}</>}</h1>
            {/* CARD PACK SECTION */}
            ...
            </div> : showCards ? <></> : <h1 className='rounded-lg bg-yellow-500 hover:cursor-pointer text-center m-14 font-extrabold' onClick={() => {
                cardPack()
